...
function test_list()
{
    ...
    let select = document.querySelector(".test_list");
    ...
    .then(response => response.json())
     .then((data)=>
     {
         ...
         select.addEventListener('change', (e) => {
          ...
          setSiGMLURL(`SignFiles/${e.target.value}.sigml`)
         })
     });
...
// displays currently playing word/letter
  function display_curr_word(word)
  {
      ...
      p.textContent=word;
      p.style="color:Red; font-decoration:bold;";
  }
  ...
  var int = setInterval(function () {
      if(i == totalWords) {
          if(playerAvailableToPlay) {
            ...
            console.log("ERROR:- ", "Sigml file of the word/letter is missing.)");
            display_err_message();
